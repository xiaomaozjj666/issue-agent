(function () {
  ...
  // C1 继续去耦：通用工具与大型职责块已拆分至独立模块
  //（core / markdown / analysis-timer / files-tracker / export），经 IA 命名空间委托；
  // 本地 const 别名保持既有调用点不变（与上方 enumLabel 同一模式）。
  ...
  const formatErrorDetail = IA.formatErrorDetail;
  const reportAsMarkdown = IA.Export.reportAsMarkdown;
  const generatePytestSkeleton = IA.Export.pytestSkeleton;
  const generateSelfContainedHtml = IA.Export.selfContainedHtml;
  const classifyRisk = IA.Export.classifyRisk;
  const classifyChangePriority = IA.Export.classifyChangePriority;
  const extractAffectedFiles = IA.Export.extractAffectedFiles;
