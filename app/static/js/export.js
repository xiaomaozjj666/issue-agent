...
(function () {
  ...
  function generateSelfContainedHtml(r, sessionData) {
    ...
    `;
    return `<!DOCTYPE html>
<html lang="${IA.escapeAttr(document.documentElement.lang || "zh")}" data-theme="dark">
<head>
<meta charset="UTF-8">
...
<body>
<button class="theme-toggle" type="button" data-label-light="${IA.escapeAttr(t("theme_light"))}" data-label-dark="${IA.escapeAttr(t("theme_dark"))}" onclick="var d=document.documentElement;var next=d.getAttribute('data-theme')==='dark'?'light':'dark';d.setAttribute('data-theme',next);this.textContent=this.getAttribute(next==='dark'?'data-label-dark':'data-label-light');document.dispatchEvent(new CustomEvent('issue-agent:report-theme'))">${IA.escapeHtml(t("theme_dark"))}</button>
