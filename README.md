...
粘贴 Issue 链接即可开始分析——深色 / 浅色双主题，桌面与移动端自适应：

| 首页 · 一键发起分析 | 调查轨迹 · 实时进度 |
|---|---|
| <img src="docs/screenshots/home-dark.png" alt="首页" width="480"> | <img src="docs/screenshots/report-dark.png" alt="调查与报告" width="480"> |

报告内置 5 个**可交互数据图表**，全部支持悬停详情与点击下钻（证据定位、风险矩阵聚焦、补丁 diff 跳转）：

- 🧩 **根因证据链** — 结论 ← 证据的树状关系，按「根因 / 影响面 / 修复方案」分组，节点颜色直接表达证据强度
- 🎯 **风险矩阵** — 严重度 × 发生可能性，标准 ISO 31010 网格，根因位置一目了然
- 🌊 **波及范围** — 受影响模块可视化（有补丁时按改动行数，无补丁时降级为语义徽章流）
- 📈 **补丁改动分布** — 与 git diff --stat 同口径的增删行统计
- ✅ **证据核对** — 引用 / 已读 / 补丁覆盖三方对账，未读引用红色警示
...
### 本地大面积失败、CI 却全绿时：先查 `NO_PROXY`

`httpx` 与 `httpx2` 会把 `NO_PROXY` 的每一项当 URL 模式解析，而**带方括号的 IPv6 字面量**（`[::1]`）会被当成通配域名，拼出非法的 `all://*[::1]`，在**构造客户端**时就抛 `InvalidURL: Invalid port: ':1]'`。`openai`（≥3.16 底层走 `httpx2`）因此在应用启动阶段就失败，表现为几十条用例一起红、且与用例内容无关。

```bash
python -c "import httpx; httpx.Client()"   # 能构造出来就说明不是这个原因
```

裸写的 `::1` 没有这个问题，两者在绕过列表里语义等价——删掉带方括号的那一项即可，`::1` 本身保留。**注意有些工具链会为迁就 undici 而同时写入 `::1` 与 `[::1]`**（undici 的匹配器会把裸写 `::1` 误读成 host `:` port `1`），所以这个值不是你手写的也可能出现。

2026-09-22 实测：同一份代码、同一台机器，`NO_PROXY=localhost,127.0.0.1,::1` 时 `579 passed / 覆盖率 97.53%`；仅追加 `,[::1]` 后同样命令 59 条失败。CI 的 runner 不带这个变量，故 CI 不受影响。
