...
async def test_read_file_returns_content_with_line_numbers() -> None:
    ...
    executor = ToolExecutor(github, _make_issue(), ["src/a.py"])
...
async def test_read_file_clamps_line_range() -> None:
    ...
    result = await executor.execute(
        "read_file", {"path": "src/a.py", "start_line": 0, "end_line": 99}
    )
...
async def test_list_directory_root() -> None:
    ...
    executor = ToolExecutor(MagicMock(), _make_issue(), tree)
...
async def test_list_directory_nonexistent() -> None:
    executor = ToolExecutor(MagicMock(), _make_issue(), ["src/a.py"])
...
async def test_grep_content_no_matches() -> None:
    executor = ToolExecutor(MagicMock(), _make_issue(), [])
...
async def test_execute_handles_file_skipped_error() -> None:
    ...
    executor = ToolExecutor(github, _make_issue(), ["src/big.py"])
...
def test_file_cache_property_returns_cache() -> None:
    executor = ToolExecutor(
        MagicMock(), _make_issue(), ["src/a.py"],
        file_cache={"src/a.py": "content"},
    )
...
def test_file_cache_copied_not_shared() -> None:
    ...
    executor = ToolExecutor(MagicMock(), _make_issue(), ["src/a.py"], file_cache=original)
...
def test_parse_tool_call_none_arguments() -> None:
    tc = SimpleNamespace(
        function=SimpleNamespace(name="list_directory", arguments=None)
    )
